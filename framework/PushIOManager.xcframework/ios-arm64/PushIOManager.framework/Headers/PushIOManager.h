...
FOUNDATION_EXPORT NSString * const PIOMessageCenterUpdateNotification;
FOUNDATION_EXPORT NSString * const PIOMCRawResponseValue;
FOUNDATION_EXPORT NSString * const PIOMCRequestURL;


@class PIOMCMessage, PIOConversionEvent;
...
@protocol PIOInAppDelegate <NSObject>

@optional

/**
Implement, this method to handle call back when inApp view is dismissed by tapping close button
*/
- (void)onInAppMessageViewClosed;
...
typedef enum {
    PIO_MC_MSG_RECEIVED_BY_APP = 0,
    PIO_MC_MSG_SAVED_TO_APP_CACHE = 1,
    PIO_MC_MSG_READ_FROM_APP_CACHE = 2,

} PIOMessageCenterEvent;
...
/**
 Set this flag to delay registrations Registration will be delayed while launching the app/from coming to background
 */
@property (nonatomic, assign) BOOL delayRegistration;
...
/**
 Method to be invoked from the scene delegate's `scene:willConnectToSession:options:` method.
 ...
 */
-(void) willConnectToSession:(UISceneSession *)session options:(UISceneConnectionOptions *)connectionOptions  API_AVAILABLE(ios(13.0));
...
/**
 Method meant to be invoked from the scene delegate's `openURLContexts:options` method.
...
@property (nonatomic, weak) id <PIOInAppDelegate> inAppDelegate;
...
/**
 Asks user permissions for provided notifications types i.e.: Sound/Badge/Alert/Critical Alert types.
 If readyForRegistrationCompHandler is not set, then provided completionHandler is assigned to it, to let application have access when SDK receives deviceToken.
...
/**
 Set OracleCX account id.
 @param accountId OracleCX account id.
*/
- (void)setOracleCXAccountId:(NSString *)accountId;
...
/**
Tracks the conversion event.

@param event conversion event to be tracked.
@param completionHandler Callback to notify when event is tracked.
*/
-(void)trackConversionEvent:(PIOConversionEvent *)event completionHandler:(PIOCompletionHandler)completionHandler;


/**
 Use this api to customise InApp Message Banner height. The maximum banner height of 200px and minimum banner height of 100px is allowed.

 @param height To customise the banner height
 @return Returns YES if banner height is set to custom height, NO otherwise
*/
-(BOOL) setInAppMessageBannerHeight:(CGFloat)height;

/**
 Returns current height of InApp Message banners.
 
 @return Returns banner height in float values
*/

-(CGFloat) getInAppMessageBannerHeight;

/*
Used to hide status bar for InApp message view. If the flag is set the status bar is hidden.

@param hideStatusBar  Set this flag to hide status bar.
*/
-(void)setStatusBarHiddenForIAMBannerInterstitial:(BOOL)hideStatusBar;

/**
 Returns whether the status bar is hidden for the InApp message view.
 
 @return Returns YES if the status bar is hidden, NO otherwise
*/
-(BOOL)isStatusBarHiddenForIAMBannerInterstitial;

/**
 set InApp message custome close button if user wants to customise close button.
 
 @param closeButton  Custom close button.
 */
-(void)setInAppMessageCloseButton:(UIButton *) closeButton;

-(void)setMessageCenterEventTrackingEnabled:(BOOL) enabled;

-(BOOL)isMessageCenterEventTrackingEnabled;

-(void)trackMessageCenterEvent:(PIOMessageCenterEvent) event messageId:(NSString *) messageId;

-(void)trackMessageCenterEvent:(PIOMessageCenterEvent) event messages:(NSArray *) messages;
